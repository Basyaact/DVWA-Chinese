...
if( isset( $_POST[ 'Login' ] ) ) {
	...
	if( $result && mysqli_num_rows( $result ) == 1 ) {    // Login Successful...
		$login_state = "<h3 class=\"loginSuccess\">无效的密码： '{$user}'</h3>";
	}else{
		// Login failed
		$login_state = "<h3 class=\"loginFail\">错误的密码： '{$user}'</h3>";
	}
...
$page[ 'body' ] .= "
		<div class=\"body_padded\">
			<h1>测试证书</h1>
			<h2>漏洞/跨站请求伪造(CSRF)</h2>
			<div id=\"code\">
				<form action=\"" . DVWA_WEB_PAGE_TO_ROOT . "vulnerabilities/csrf/test_credentials.php\" method=\"post\">
					<fieldset>
						" . $login_state . "
						<label for=\"user\">用户名</label><br /> <input type=\"text\" class=\"loginInput\" size=\"20\" name=\"username\"><br />
						<label for=\"pass\">密码</label><br /> <input type=\"password\" class=\"loginInput\" AUTOCOMPLETE=\"off\" size=\"20\" name=\"password\"><br />
						<p class=\"submit\"><input type=\"submit\" value=\"点击登录\" name=\"Login\"></p>
					</fieldset>
				</form>
				{$messagesHtml}
			</div>
